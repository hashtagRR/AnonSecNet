...
int packet_sphinx_peel(const wire_packet_t *pkt,
    ...
    /*
     * Layout of the OUTPUT wire packet (for the next hop):
     *
     *   [0 .. remaining_header-1]  : remaining header blocks (hops 1..N-1)
     *                                = (N_PATH_HOPS-1) x HEADER_BLOCK_SIZE
     *                                = 2 x 87 = 174 bytes
     *   [remaining_header .. 1023] : blinded payload (763 bytes)
     *                                + zero-pad of one freed block (87 bytes)
     *                                so total wire size stays 1024 bytes.
     *
     * BUG FIXED: previous code placed payload at offset HEADER_SIZE (261),
     * skipping the freed 87-byte header slot entirely and leaving bytes
     * 174..260 as zero, causing the next hop to read garbage header data.
     * Correct offset is HEADER_SIZE - HEADER_BLOCK_SIZE = 174.
     */
    memset(out_pkt->data, 0, WIRE_PACKET_SIZE);

    size_t remaining_header = HEADER_SIZE - HEADER_BLOCK_SIZE; /* 174 bytes */
    if (remaining_header > 0)
        memcpy(out_pkt->data,
               pkt->data + HEADER_BLOCK_SIZE,
               remaining_header);

    /* Payload immediately follows the shifted header at offset 174         */
    memcpy(out_pkt->data + remaining_header, payload_out, PAYLOAD_SECTION_SIZE);
