...
#include <stdlib.h>
#include <windows.h>
static void log_msg(const char *m){printf("[SndEnt] %s\n",m);fflush(stdout);}
static void log_err(const char *m){fprintf(stderr,"[SndEnt] ERROR: %s\n",m);fflush(stderr);}
...
static DWORD WINAPI handle_connection(LPVOID arg){
    ...
    if(mode==MODE_DATA){
        ...
        log_msg("forwarding sender packet to MN1");
...
int main(void){
    ...
    SOCKET srv=net_listen(PORT_SENDER_ENTRY);
